...
template<class iRead, class iTable>
iRead ReadFunc(iTable* table, int index) {
	auto tabledata = reinterpret_cast<uintptr_t**>(table);
	auto tableread = *tabledata;
	return *reinterpret_cast<iRead>(tableread[index]);
}
